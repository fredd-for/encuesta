    ...
    <form class="formA validable" action="acap15a3Add.php" method="post" autocomplete="off" >
      <fieldset>                          
          ...
          <table width="100%" class="fOpt" id="tablecertificacion" >
                ...
                <tbody>
                <?php
                $posmax = OPERATOR::getDbValue("SELECT MAX(prod_position) + 1 as pos FROM frm2_cap15_productosterminados WHERE prod_regi_uid = '".$regisroUID."' AND  prod_tima_uid = '9' AND prod_position <> 0");
                if( empty($posmax) ) { $posmax = 1; }
                
                $sql3 = "SELECT * FROM frm2_cap15_productosterminados WHERE prod_regi_uid = '".$regisroUID."' AND prod_delete = 0 AND prod_position <> 0 AND prod_tima_uid = '9' ORDER BY prod_position ASC  ";
                //echo $sql3;
                ...
                $sql = "SELECT
                        ...
                        FROM frm2_cap15_productosterminados WHERE prod_regi_uid = '".$regisroUID."' AND prod_delete = 0 AND  prod_tima_uid IN (7,8,9) AND prod_position <> 0 GROUP BY prod_regi_uid";
                $db3->query( $sql );                
