    ...
    <form class="formA validable" action="acap14a2Add.php" method="post" autocomplete="off" >
      <fieldset>                          
          ...
          <table width="100%" class="fOpt" id="tablecertificacion" >
                <thead>
                <tr>
                  <td rowspan="2" align="center" >DESCRIPCI&Oacute;N GEN&Eacute;RICA DE MATERIALES AUXILIARES</td>
                ...
                <tbody>
                <?php
                $posmax = OPERATOR::getDbValue("SELECT MAX(mapr_position) + 1 as pos FROM frm2_cap14a_materiasprimas WHERE mapr_regi_uid = '".$regisroUID."' AND  mapr_tima_uid = '2' AND mapr_position <> 0");                
                if( empty($posmax) ) { $posmax = 1; }
                
                $sql3 = "SELECT * FROM frm2_cap14a_materiasprimas WHERE mapr_regi_uid = '".$regisroUID."' AND mapr_delete = 0 AND  mapr_tima_uid = '2' AND mapr_position <> 0 ORDER BY mapr_position ASC  ";
                //echo $sql3;
