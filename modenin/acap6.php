...
if( $db->numrows() == 0 ) {
    ...
    while( $aDefinicion =  $db2->next_record() ) {
        $sql = "INSERT INTO frm2_cap6_ventaproductos SET ";
...
?>
    ...
    <tbody>
        <tr>                        
            <td colspan="2"><?php echo OPERATOR::getDescTitles(2,'A',6,0); ?></td>
        </tr>
    </tbody>
    </table>
        ...
        <fieldset>
          ...
          <table width="100%" class="fOpt"  >
                ...
                $sql = " SELECT vepr_defi_uid, vepr_valor "
                      ." FROM frm2_cap6_ventaproductos "
                      ." WHERE vepr_regi_uid = '".$regisroUID."' AND vepr_defi_uid IN (158,159,151,138,143,137) ";                
                $db->query( $sql );                
                //echo $sql;
                $tot1 = 0;                
                $tot2 = 0;
                ...
                while( $aVP = $db->next_record() ) {
                    if( $aVP["vepr_defi_uid"]==158 ) { $tot1 = $aVP["vepr_valor"]; } //Ventas a Instituciones Públicas
                    if( $aVP["vepr_defi_uid"]==159 ) { $tot2 = $aVP["vepr_valor"]; } //Ventas a Empresas privadas
                    if( $aVP["vepr_defi_uid"]==151 ) { $tot3 = $aVP["vepr_valor"]; } //Ventas a Personas particulares
                    if( $aVP["vepr_defi_uid"]==138 ) { $tot4 = $aVP["vepr_valor"]; } //Total Ventas al Mercado Nacional
                    if( $aVP["vepr_defi_uid"]==143 ) { $tot5 = $aVP["vepr_valor"]; } //Total Ventas al Mercado Externo
                    if( $aVP["vepr_defi_uid"]==137 ) { $tot6 = $aVP["vepr_valor"]; } //TOTAL VENTAS
                    
                }
                
                $sql = " SELECT frm2_cap6_ventaproductos.*, adm_definiciones.defi_vinieta as vinieta, adm_definiciones.defi_indent	 as indent "
                      ." FROM frm2_cap6_ventaproductos LEFT JOIN  adm_definiciones ON ( vepr_defi_uid	 = defi_uid ) "
                      ." WHERE vepr_regi_uid = '".$regisroUID."' AND vepr_defi_uid IN (139,140,141,142,144,145,146,147,148,149,150,152,153) "
                ...
                <tr>
                  ...
                  <td align="center"  >
                  <input   type="text" name="input-28" id="input-28" onblur="javascript:saveUPD(2); return false;" value="<?php echo $aVP["vepr_valor"]; ?>" size="20" maxlength="3" class="inpB2 numeric" />
                ...
                <tr>
                  ...
                  <td align="center"  >
                      <input type="text" name="input-29" id="input-29"  value="<?php echo $aVP["vepr_valor"]; ?>" size="20" maxlength="3" class="inpB2 numeric" />
                  </td>
                </tr>
                <?php 
                    $percent2 = $percent2 + $aVP["vepr_valor"];
                ...
                <tr>
                  ...
                  <td align="center"  ><input   type="text" name="input-31" id="input-31" onblur="javascript:saveUPD(3); return false;" value="<?php echo $aVP["vepr_valor"]; ?>" size="20" maxlength="3" class="inpB2 numeric" /></td>
                </tr>
                <tr>
                  <td align="left" class="titR" >Total %</td>
                  <td align="center"  >
