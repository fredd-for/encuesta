...
if( !empty($regisroUID) && !empty($uidFormulario) ) {
    ...
    $tabla = OPERATOR::toSql(safeHTML(OPERATOR::getParam("tabla")),'Number');
    if( !empty($tabla) ) {
    /*------------------------------------------------------------------------*/        
    $reg = $tabla;    
    switch( $tabla ){
    ...
    $sql = "SELECT * FROM frm2_dcap1a_capacitacion WHERE capa_regi_uid = '".$regisroUID."' AND capa_defi_uid = '".$capa_defi_uid."' AND capa_position <> 0 ";          
    $db2->query($sql);
    $aPosition = array();
    ...
    if( $db2->numrows() >  0 ) {
        ...
        while( $aCert = $db2->next_record()  ) {
            ...
            if( $aCert["capa_delete"] != 1 ) {
                ...
                if( !empty($colA)  ) { 
                    $sql  = "UPDATE frm2_dcap1a_capacitacion SET ";                    
                    $sql .= "capa_descripcion = UPPER('".utf8_decode($colA)."'), "; 
                    $sql .= "capa_valor	 = '".$colB."', ";
                    $sql .= "capa_institucion = UPPER('".utf8_decode($colC)."'), ";                                                                                 
    ...
    for( $pos = 1; $pos<=$maxrow; $pos++ ) {
        ...
        if( !in_array($pos, $aPosition) ) {
            ...
            if( !empty($colA)  ) {                                
                $sql  = "INSERT frm2_dcap1a_capacitacion SET ";
    ...
    $capa_defi_uid = 307;
    $sql = "SELECT * FROM frm2_dcap1a_capacitacion WHERE capa_regi_uid = '".$regisroUID."' AND capa_defi_uid = '".$capa_defi_uid."' ";          
    $db2->query($sql);
    if( $db2->numrows() >  0 ) {        
        $sql  = "UPDATE frm2_dcap1a_capacitacion SET ";
