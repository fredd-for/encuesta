...
<script type="text/javascript" src="js/acap15a_2.js"></script>
<?php 
/* usuario */
...
<div class="content">
    ...
    <tbody>
        <tr>                        
            <td colspan="2"><?php echo OPERATOR::getDescTitles(2,'A',15,0); ?></td>
        </tr>
    </tbody>
    </table>        
    
    <form class="formA validable" action="acap15a2Add.php" method="post" autocomplete="off" >
      <fieldset>                          
          <p> <span class="subT">SUBPRODUCTOS</span></p>
          <table width="100%" class="fOpt" id="tablecertificacion" >
                <thead>
                <tr>
                  <td rowspan="2" align="center" >DESCRIPCI&Oacute;N DE SUBPRODUCTOS</td>
                ...
                <tbody>
                <?php
                $posmax = OPERATOR::getDbValue("SELECT MAX(prod_position) + 1 as pos FROM frm2_cap15_productosterminados WHERE prod_regi_uid = '".$regisroUID."' AND  prod_tima_uid = '8' AND prod_position <> 0");
                if( empty($posmax) ) { $posmax = 1; }
                
                $sql3 = "SELECT * FROM frm2_cap15_productosterminados WHERE prod_regi_uid = '".$regisroUID."' AND prod_delete = 0 AND prod_position <> 0 AND prod_tima_uid = '8' ORDER BY prod_position ASC  ";
                //echo $sql3;
                ...
                <tr>
                  ...
                  <td align="right"  ><label class="labB" id="totL"><?php echo number_format($l) ?></td>
                  <td align="right"  ><label class="labB" id="totM"><?php echo number_format($m) ?></td>
                  <td align="right"  >&nbsp;</td>
                </tr>
                </tfoot>
            ...
            <p></p>
            <p><?php echo OPERATOR::getDescTitles(2,'A',15,1); ?></p>
            <span class="bxBt">
            <input type="submit" value="SIGUIENTE" id="sendData" name="continuarregistro" class="button" >
            <a href="acap15a1.php" class="btnS">ANTERIOR</a>                
            </span>
