    ...
    <form class="formA validable" action="dcap1a3Add.php" method="post" autocomplete="off" >
      <fieldset>          
          ...
          <table width="100%" class="fOpt" id="table_a" >
                ...
                <tbody>
                <?php
                $posmax = OPERATOR::getDbValue("SELECT MAX(capa_position) + 1 as pos FROM frm2_dcap1a_capacitacion WHERE capa_regi_uid = '".$regisroUID."' AND capa_defi_uid = '292' AND capa_position <> 0");                
                if( empty($posmax) ) { $posmax = 1; }
                $sql3 = "SELECT * FROM frm2_dcap1a_capacitacion WHERE capa_defi_uid = '292' AND capa_regi_uid = '".$regisroUID."' AND capa_delete = 0 AND capa_position <> 0 ORDER BY capa_position ASC ";
