...
if( !empty($regisroUID) && !empty($uidFormulario) ) {
    ...
    $sql = "SELECT * FROM frm2_cap14a_materiasprimas WHERE mapr_regi_uid = '".$regisroUID."' AND mapr_tima_uid = '".$mapr_tima_uid."' AND mapr_position <> 0 ";          
    $db2->query($sql);
    $aPosition = array();
    ...
    if( $db2->numrows() >  0 ) {
        ...
        while( $aCert = $db2->next_record()  ) {
            ...
            if( $aCert["mapr_delete"] != 1 ) {
                ...
                $sql .= "mapr_proveedor = UPPER('".utf8_decode($actividad)."'), "; 
    ...
    for( $pos = 1; $pos<=$maxrow; $pos++ ) {
        ...
        if( !in_array($pos, $aPosition) ) {                        
            ...
            if( !empty($desc) && !empty($actividad)  ) {                                
                $sql  = "INSERT frm2_cap14a_materiasprimas SET ";
    ...
    // totales
    $uidtotalmateria = 4;
    $sql = "SELECT mapr_uid FROM frm2_cap14a_materiasprimas WHERE mapr_position = '0' AND mapr_tima_uid = '".$uidtotalmateria."' AND mapr_regi_uid = '".$regisroUID."' ";
    $db->query( $sql );
