...
if( $db->numrows() == 0 ) {
    ...
    while( $aDefinicion =  $db2->next_record() ) {
        $sql = "INSERT INTO  frm2_bcap2_sistemagestion SET ";
...
<?php            
    ...
    $sql = " SELECT  frm2_bcap2_sistemagestion.*, adm_definiciones.defi_subcapitulo as subcap, adm_definiciones.defi_indent as indent, adm_definiciones.defi_vinieta as vinieta "
          ." FROM  frm2_bcap2_sistemagestion LEFT JOIN  adm_definiciones ON ( sige_defi_uid = defi_uid ) "
          ." WHERE sige_regi_uid = '".$regisroUID."' AND sige_defi_uid IN (234, 235, 236 )  "
    ...
    $sql = " SELECT  frm2_bcap2_sistemagestion.*, adm_definiciones.defi_subcapitulo as subcap, adm_definiciones.defi_indent as indent, adm_definiciones.defi_vinieta as vinieta  "
          ." FROM  frm2_bcap2_sistemagestion LEFT JOIN adm_definiciones ON ( sige_defi_uid = defi_uid ) "
          ." WHERE sige_regi_uid = '".$regisroUID."' AND sige_defi_uid NOT IN (234, 235, 236 ) "
...
<div class="content">
    ...
    <form class="formA validable" action="bcap2Add.php" method="post" autocomplete="off" >
        <fieldset>
            ...
            <p>
            <?php echo OPERATOR::getDescTitles(2,'B',2,'1'); ?>
            </p>
            <p>
            ...
            <p>
                <?php echo OPERATOR::getDescTitles(2,'B',2,'2'); ?>                
            </p>
            <p>
            ...
            <tbody>
                ...
                //echo "SELECT MAX(prod_position) + 1 as pos FROM  frm2_bcap2_productos WHERE prod_regi_uid = '".$regisroUID."' AND prod_position <> 0";
                if( empty($posmax) ) { $posmax = 1; }
                
                $sql3 = "SELECT * FROM  frm2_bcap2_productos WHERE prod_regi_uid = '".$regisroUID."' AND prod_delete = 0 ORDER BY prod_position ASC  ";
                //echo $sql3;                
                $db3->query( $sql3 );
                if( $db3->numrows() > 0 ) {
