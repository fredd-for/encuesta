...
if( $db->numrows() == 0 ) {
    ...
    while( $aDefinicion =  $db2->next_record() ) {
        $sql = "INSERT INTO  frm2_cap12_capitalsocial SET ";
...
$sql = " SELECT frm2_cap12_capitalsocial.*, adm_definiciones.defi_subcapitulo as subcap, adm_definiciones.defi_indent  as indent  "
      ." FROM frm2_cap12_capitalsocial LEFT JOIN  adm_definiciones ON ( caso_defi_uid	 = defi_uid ) "
...
<div class="content">
    ...
    <tbody>
        <tr>                        
            <td colspan="2"><?php echo OPERATOR::getDescTitles(2,'A',12,0); ?></td>
        </tr>
    </tbody>
    </table>        
      ...
      <fieldset>
        ...
        <table width="100%" class="fOpt" >
            <thead>
                ...
                <?php
                $sql = " SELECT frm2_cap12_capitalsocial.*, adm_definiciones.defi_subcapitulo as subcap  "
                ." FROM frm2_cap12_capitalsocial LEFT JOIN  adm_definiciones ON ( caso_defi_uid	 = defi_uid ) "
                ." WHERE caso_regi_uid = '".$regisroUID."' AND  defi_uid IN (191,192) "
