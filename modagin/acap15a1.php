    ...
    <form class="formA validable" action="acap15a1Add.php" method="post" autocomplete="off" >
      <fieldset>                          
          ...
          <table width="100%" class="fOpt" id="tablecertificacion" >
                ...
                <tbody>
                <?php
                $posmax = OPERATOR::getDbValue("SELECT MAX(prod_position) + 1 as pos FROM frm3_cap15_productosterminados WHERE prod_regi_uid = '".$regisroUID."' AND  prod_tima_uid = '7' AND prod_position <> 0");
                if( empty($posmax) ) { $posmax = 1; }
                
                $sql3 = "SELECT * FROM frm3_cap15_productosterminados WHERE prod_regi_uid = '".$regisroUID."' AND prod_delete = 0 AND prod_position <> 0 AND prod_tima_uid = '7' ORDER BY prod_position ASC  ";
                //echo $sql3;
