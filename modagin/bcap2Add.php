...
$dat[458] = OPERATOR::toSql(safeHTML(OPERATOR::getParam("rbtn_inv")),'Number');
...
$desc[461] = OPERATOR::toSql(safeHTML(OPERATOR::getParam("inversionotros")),'Text');
if( $chk_3 == 0 ) {
    $desc[461] = "";
}
...
$dat[469] = OPERATOR::toSql(safeHTML(OPERATOR::getParam("rbtn_gastos")),'Number');  // 1
...
if( !empty($regisroUID)  ) { 
    ...
    while ( $aGesAmb = $db->next_record() ) {         
        $sql  = "UPDATE frm3_bcap2_sistemagestion SET ";
        $sql .= "sige_valor = '".$dat[$aGesAmb["sige_defi_uid"]]."', ";
        $sql .= "sige_description = UPPER('".$desc[$aGesAmb["sige_defi_uid"]]."'), ";
    ...
    if( !empty($regisroUID) && !empty($uidFormulario) && $dat[469] == 1 ) {        
    
    //actualizar existentes
    $sql = "SELECT * FROM frm3_bcap2_productos WHERE prod_regi_uid = '".$regisroUID."' AND prod_position <> 0 ";          
    $db2->query($sql);
    $aPosition = array();
    ...
    if( $db2->numrows() >  0 ) {
        ...
        while( $aCert = $db2->next_record()  ) {
            ...
            if( $aCert["prod_delete"] != 1 ) {
                ...
                if( !empty($desc) ) {
                $sql  = "UPDATE frm3_bcap2_productos SET ";                     
                $sql .= "prod_description	 = UPPER('".$desc."'), ";                      
    ...
    for( $pos = 1; $pos<=$maxrow; $pos++ ) {
        ...
        if( !in_array($pos, $aPosition) ) {                      
            ...
            if( !empty($desc) ) {                           
                $sql  = "INSERT frm3_bcap2_productos SET ";
