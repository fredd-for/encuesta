...
switch ( $reg  ){
    case 'a': $capa_defi_uid = 513; break;
    case 'b': $capa_defi_uid = 514; break;
    case 'c': $capa_defi_uid = 515; break;    
}
...
$sql = "SELECT SUM(capa_valor) as tot FROM frm3_dcap1a_capacitacion WHERE capa_regi_uid = '".$regisroUID."' AND capa_defi_uid IN(513, 514, 515) AND capa_delete = 0 ";
...
$sql = " UPDATE frm3_dcap1a_capacitacion SET capa_valor = '".$totalExistente."', capa_suv_uid = '".$uid_token."', capa_updatedate = NOW() "
      ." WHERE capa_regi_uid = '".$regisroUID."' AND capa_defi_uid = '532'  ";
$db->query( $sql );
