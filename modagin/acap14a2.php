...
<script type="text/javascript" src="js/acap14a_2.js"></script>
<?php 
/* usuario */
...
<div class="content">
    ...
    <form class="formA validable" action="acap14a2Add.php" method="post" autocomplete="off" >
      <fieldset>                          
          <p> <span class="subT">MATERIALES AUXILIARES E INSUMOS</span> </p>
          <table width="100%" class="fOpt" id="tablecertificacion" >
                <thead>
                <tr >
                  <td rowspan="3" align="center" >Descripci&oacute;n gen&eacute;rica de materiales auxiliares</td>
                ...
                <tbody>
                <?php
                $posmax = OPERATOR::getDbValue("SELECT MAX(mapr_position) + 1 as pos FROM frm3_cap14a_materiasprimas WHERE mapr_regi_uid = '".$regisroUID."' AND  mapr_tima_uid = '2' AND mapr_position <> 0");
                if( empty($posmax) ) { $posmax = 1; }
                
                $sql3 = "SELECT * FROM frm3_cap14a_materiasprimas WHERE mapr_regi_uid = '".$regisroUID."' AND mapr_delete = 0 AND mapr_position <> 0 AND mapr_tima_uid = '2' ORDER BY mapr_position ASC  ";
                //echo $sql3;
            ...
            <a href="acap14a.php" class="btnS">ANTERIOR</a>                
            </span>
