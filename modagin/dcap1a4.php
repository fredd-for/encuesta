...
<script type="text/javascript" src="js/dcap1a4.js"></script>
<?php 
/* usuario */
...
<div class="content">
    ...
    <form class="formA validable" action="dcap1a4Add.php" method="post" autocomplete="off" >
      <fieldset>          
          <!-- table 1 -->           
          <p> <span class="subT">4. EVENTOS DE CAPACITACI&Oacute;N ESPECIALIZADA Y TRANSFERENCIA DE TECNOLOG&Iacute;A A PROVEEDORES DE MATERIAS PRIMAS Y SERVICIOS</span> </p>
          <table width="100%" class="fOpt" id="table_a" >
                ...
                <tbody>
                <?php
                $posmax = OPERATOR::getDbValue("SELECT MAX(capa_position) + 1 as pos FROM frm3_dcap1a_capacitacion WHERE capa_regi_uid = '".$regisroUID."' AND capa_defi_uid = '518' AND capa_position <> 0");                
                if( empty($posmax) ) { $posmax = 1; }
                $sql3 = "SELECT * FROM frm3_dcap1a_capacitacion WHERE capa_defi_uid = '518' AND capa_regi_uid = '".$regisroUID."' AND capa_delete = 0 AND capa_position <> 0 ORDER BY capa_position ASC ";
            ...
            <a href="dcap1a3.php" class="btnS">ANTERIOR</a>                
            </span>
